...
const getAllFaq = (filters, paginationOptions) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (searchTerm) {
        andConditions.push({
            $or: sectionFaq_constant_1.sectionFaqSearchableFields.map((field) => ({
                [field]: {
                    $regex: searchTerm,
                    $options: "i",
                },
    ...
    const result = yield sectionFaq_model_1.SectionFaq.find(whereConditions)
        .sort(sortConditions)
        .skip(skip)
        .limit(limit);
    const total = yield sectionFaq_model_1.SectionFaq.countDocuments(whereConditions);
    return {
        meta: {
            page,
            limit,
